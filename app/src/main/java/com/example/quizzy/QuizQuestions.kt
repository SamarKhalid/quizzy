...
class QuizQuestions : AppCompatActivity() , View.OnClickListener{
            ...
            R.id.btn_next -> {
                if (selectedOption == 0) {
                    ...
                    when {
                        ...
                        Snackbar.make(view, "Questions done", Snackbar.LENGTH_LONG).show()
                    }
                    }
                } else {
    ...
    /*private fun nextOnClick(){
        ...
        nextBtn.setOnClickListener{
            if(currentPosition < questionList.size){
            currentPosition++
            val currentQuestion : Question = questionList[currentPosition]
                progressBar?.progress = currentPosition
                tvProgress?.text = "$currentPosition/${progressBar?.max}"
                tvQuestion?.text = currentQuestion.question
                imgQuestion?.setImageResource(currentQuestion.img)
                ...
                Snackbar.make(it, "Questions done", Snackbar.LENGTH_LONG).show()
